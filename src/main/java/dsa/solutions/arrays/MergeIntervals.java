    ...
    */
    ...
    static int[][] mergeIntervals(int[][] intervals){
        ...
        for(int i = 1 ; i< n ; i++){
            ...
            if( e >= intervals[i][0] ){   //it means current interval overlaps with last interval of merged
                //so merge them

                e = Math.max( e , intervals[i][1] );
                merged.get(k).set(1,e);
            }
            else{   //it means current interval doesn't overlaps with last interval of merged
